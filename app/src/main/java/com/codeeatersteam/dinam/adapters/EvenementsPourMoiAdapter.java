...
/**
 * Created by pondikpa on 19/09/17.
 */

public class EvenementsPourMoiAdapter extends RecyclerView.Adapter<EvenementsPourMoiAdapter.ViewHolder> {
    private ArrayList<EvenementsDao> daoList;
    private Context context;

    public EvenementsPourMoiAdapter(ArrayList<EvenementsDao> daoList, Context context) {
        this.daoList = daoList;
        this.context = context;
    }

    @Override
    public EvenementsPourMoiAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_evenement,parent,false);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(EvenementsPourMoiAdapter.ViewHolder holder, int position) {
